...
pedocs_clean <- pedocs %>%
  ...
  mutate(oa_urls = map_chr(oa_urls, ~ toJSON(list(pdf_links = .x), auto_unbox = FALSE)))
...
pedocs_final <- pedocs_clean %>%
  ...
  mutate(openalex_id = if_else(!is.na(id), id, paste("extern", `Source-Opus`))) %>%
  select(openalex_id, journal_id, journal_name, doi, publication_year,
         oa_urls, pdf_download_status, pdf_local_path,
...
if (length(missing_cols) > 0) {
  ...
  stop(paste("The following columns are missing from the DB:", paste(missing_cols, collapse = ", ")))
}

dbBegin(conn_pd)
tryCatch({
  dbWriteTable(conn_pd, "works", pedocs_final, append = TRUE, row.names = FALSE)
  dbCommit(conn_pd)
