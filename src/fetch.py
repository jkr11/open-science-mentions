...
def get_journal_by_id(
  ...
  min_year: int,
  pdf: bool = True,
  field_id: Optional[str] = None,  # New: e.g., "33"
  subfield_id: Optional[str] = None,  # New: e.g., "3304"
  _cursor=None,
) -> Generator[Any, None, None]:
  ...
  while True:
    filter_parts = [
      f"primary_location.source.id:{'|'.join(journal_ids)}",
      "open_access.is_oa:true",
      f"has_content.pdf:{str(pdf).lower()}",
      f"publication_year:>{min_year}",
      "type:article",
    ]
